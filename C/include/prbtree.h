...
struct _prbtree_node {
	union {
		...
		struct _prbtree_node *leaves[2]; /* left, right */
	} u;
...
static inline struct _prbtree_node *_prbtree_node_from_btree_node(
	A_In_opt const struct _btree_node *n/*NULL?*/)
{
	union {
		const struct _btree_node *n;
		struct _prbtree_node *pn;
	} u;
	u.n = n;
	return u.pn;
}
...
static inline struct _btree_node *_prbtree_node_to_btree_node(
	A_In_opt const struct _prbtree_node *pn/*NULL?*/)
{
	union {
		const struct _prbtree_node *pn;
		struct _btree_node *n;
	} u;
	u.pn = pn;
	return u.n;
...
struct _prbtree {
	struct _prbtree_node *root; /* NULL if tree is empty */
};
...
A_Nonnull_all_args
static inline void _prbtree_init_node(A_Out struct _prbtree_node *e)
{
	e->prbtree_left = (struct _prbtree_node*)0;
	e->prbtree_right = (struct _prbtree_node*)0;
	e->parent_color = (char*)0;
}

#ifdef ASSERT
A_Nonnull_all_args A_Pure_function
static inline void _prbtree_check_new_node(A_In const struct _prbtree_node *e)
{
...
static inline struct _prbtree_node *PRB_GET_PARENT(A_In const struct _prbtree_node *n)
{
	char *p = n->parent_color;
	return (struct _prbtree_node*)(p - (unsigned)(1u & (p - (const char*)0)));
}

A_Const_function A_Check_return
static inline char *_PRB_MAKE_PARENT_COLOR(
	A_In_opt struct _prbtree_node *p/*NULL?*/,
...
#define PRB_BLACK_NODE_PARENT(n)    ((struct _prbtree_node*)(n)->parent_color)
...
static inline void _prbtree_insert(
	A_In struct _prbtree *tree/*!=NULL*/,
	A_In struct _prbtree_node *A_Restrict p/*NULL?*/,
	A_In struct _prbtree_node *A_Restrict e/*!=NULL*/,
	int c)
{
...
static inline void __prbtree_replace_at_parent(
	A_In struct _prbtree *tree/*!=NULL*/,
	A_In_opt struct _prbtree_node *A_Restrict p/*NULL?*/,
	A_In const struct _prbtree_node *A_Restrict o/*!=NULL*/,
	A_In struct _prbtree_node *A_Restrict e/*!=NULL*/)
{
	if (!p)
		tree->root = e;
	...
	A_In struct _prbtree_node *A_Restrict e/*!=NULL*/)
{
	...
	{
		struct _prbtree_node *A_Restrict a = o->prbtree_left;
		if (a)
			PRB_SET_PARENT_COLOR(a, e, PRB_GET_COLOR(a));
	}
	{
		struct _prbtree_node *A_Restrict b = o->prbtree_right;
		if (b)
...
static inline struct _prbtree_node *_prbtree_right_parent(A_In const struct _prbtree_node *current)
{
	for (;;) {
		struct _prbtree_node *p = PRB_GET_PARENT(current);
...
A_Nonnull_all_args A_Pure_function A_Check_return A_Ret_maybenull
static inline struct _prbtree_node *_prbtree_left_parent(A_In const struct _prbtree_node *current)
{
	for (;;) {
		struct _prbtree_node *p = PRB_GET_PARENT(current);
...
A_Nonnull_all_args A_Pure_function A_Check_return A_Ret_maybenull
static inline struct _prbtree_node *_prbtree_next(A_In const struct _prbtree_node *current)
{
	const struct _prbtree_node *n = current->prbtree_right;
	if (n)
...
A_Nonnull_all_args A_Pure_function A_Check_return A_Ret_maybenull
static inline struct _prbtree_node *_prbtree_prev(A_In const struct _prbtree_node *current)
{
	const struct _prbtree_node *p = current->prbtree_left;
	if (p)
